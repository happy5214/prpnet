...
void CarolKyneaHTML::ServerStats(void)
{
   ...
   int32_t     summaryBases = 0, summaryCountInGroup = 0, summaryMinInGroup = 999999999;
   ...
   const char *theSelect = "select b, c, CountInGroup, MinInGroup, MaxInGroup, " \
   ...
   do
   {
      ...
      if (ib_ServerStatsSummaryOnly) continue;
   ...
   ip_Socket->Send("<th scope=\"row\">Bases: %d</th>", summaryBases);
